...
* **敏感操作双重锁屏**：锁屏期间检测到 Windows 键、Alt+Tab、Alt+Esc 等敏感操作时，自动调用系统锁屏作为补充防护。
...
* 需要输入正确的解锁密码才能解除锁定。
* 拦截 Alt+F4 防止绕过锁定。
* 检测到 Windows 键、Alt+Tab、Alt+Esc 等敏感操作时，自动调用系统锁屏作为补充防护（保留自定义锁屏窗口）。

> 注意：Ctrl+Alt+Del 属 Windows 安全注意序列，由内核强制处理，无法被普通程序拦截或检测；Windows 键、Alt+Tab 等则会在检测到后触发系统锁屏兜底。
...
主窗口采用深色现代风格界面（深色背景 + 圆角卡片 + 浅色文字），顶部为按钮区、中部为监控开关区、底部为告警日志区。
